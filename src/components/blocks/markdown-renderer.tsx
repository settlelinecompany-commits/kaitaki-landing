  ...
  const formatContent = (text: string) => {
    ...
    return processedText
      ...
      .replace(/^# (.*$)/gim, (match, title) => {
        ...
        return `<h1 id="${id}" class="text-4xl font-bold text-zinc-100 mb-6 mt-8">${title}</h1>`;
      })
      .replace(/^## (.*$)/gim, (match, title) => {
        const id = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
        return `<h2 id="${id}" class="text-3xl font-bold text-zinc-100 mb-4 mt-8">${title}</h2>`;
      })
      .replace(/^### (.*$)/gim, (match, title) => {
        const id = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
        return `<h3 id="${id}" class="text-2xl font-semibold text-zinc-100 mb-3 mt-6">${title}</h3>`;
      })
      .replace(/^#### (.*$)/gim, (match, title) => {
        const id = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
        return `<h4 id="${id}" class="text-xl font-semibold text-zinc-100 mb-2 mt-4">${title}</h4>`;
      })
      ...
      // Lists
      .replace(/^\- (.*$)/gim, '<li class="ml-4 mb-2 text-zinc-300">$1</li>')
      .replace(/(<li[^>]*>.*?<\/li>)/g, '<ul class="list-disc list-inside mb-4 space-y-2 ml-4">$1</ul>')
      
      // Paragraphs
      .replace(/\n\n/g, '</p><p class="mb-4 leading-relaxed text-zinc-300">')
      .replace(/^(?!<[h|d|u|l|b])/gm, '<p class="mb-4 leading-relaxed text-zinc-300">')
      .replace(/(?<!>)$/gm, '</p>')
      
      // Clean up empty paragraphs
      .replace(/<p class="mb-4 leading-relaxed text-zinc-300"><\/p>/g, '')
      .replace(/<p class="mb-4 leading-relaxed text-zinc-300">\s*<\/p>/g, '')
      // Clean up blockquotes wrapped in paragraphs
      .replace(/<p class="mb-4 leading-relaxed text-zinc-300">(<blockquote[^>]*>.*?<\/blockquote>)<\/p>/g, '$1');
  };
